...
def test(ctx):
    ...
    ctx.tester.runCommand("DeepSleep",POWERON_DELAY + 1)
    amp = ctx.sourcemeter.ampTest() * 1000
    ctx.logger.info("I_dsleep amp is %f mA when VCC is 5v"%amp)

    ctx.sourcemeter.applyVoltage(3.3)
    time.sleep(POWERON_DELAY)
    amp = ctx.sourcemeter.ampTest() * 1000
    ctx.logger.info("I_dsleep amp is %f mA when VCC is 3.3v"%amp)

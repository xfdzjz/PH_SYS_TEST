...
def test(ctx):
    ...
    while resp !='end':
        ...
        if resp == 'ready':
            ...
            fre = para[1]
            duty = para[0]
            ctx.logger.info("VCC is 3.3v fre is %f K, duty is %f" %(fre/1000,duty))
            ctx.powersupply.voltageOutput(3, 2.2, 0.1, 5, 1)
            para = ctx.oscilloscope.paraTest(2)
            fre = para[1]
            duty = para[0]
            ctx.logger.info("VCC is 2.2v fre is %f K, duty is %f" %(fre/1000,duty))
            ctx.powersupply.voltageOutput(3, 5, 0.1, 5, 1)
            para = ctx.oscilloscope.paraTest(2)
            fre = para[1]
            duty = para[0]
            ctx.logger.info("VCC is 5v fre is %f K, duty is %f" %(fre/1000,duty))
        else:
            return False

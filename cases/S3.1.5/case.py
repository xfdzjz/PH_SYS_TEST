...
def test(ctx):
    ...
    ctx.tester.runCommand("Sleep",POWERON_DELAY+1)
    amp = ctx.sourcemeter.ampTest() * 1000
    ctx.logger.info("I_sleep amp is %f mA when VCC is 5v"%amp)

    ctx.sourcemeter.applyVoltage(3.3)
    time.sleep(POWERON_DELAY)
    amp = ctx.sourcemeter.ampTest() * 1000
    ctx.logger.info("I_sleep amp is %f mA when VCC is 3.3v"%amp)

    ctx.sourcemeter.applyVoltage(2.2)
    time.sleep(POWERON_DELAY)
    amp = ctx.sourcemeter.ampTest() * 1000
    ctx.logger.info("I_sleep amp is %f mA when VCC is 2.2v"%amp)
    return True

...
def test(ctx):
    ...
    ctx.oscilloscope.trigMul(2,'POS',0.7, scale = 1,vscale= 1)
    #ctx.oscilloscope.trigSlope(1,"PGReater",0.2,1.5,0.6)
    ctx.sourcemeter.rampvol(0,3.3,0.2,0.05)
    ...
    for i in range(0,len(porWave)):
        if float(porWave[i]) >= 0.9 :
            ctx.logger.info ('VCC %f V when POR greater than 0.9' % float(vccWave[i]))
            break
    ...
    ctx.oscilloscope.trigMul(2,'NEG',0.7,scale = 1,vscale= 1)
    #ctx.oscilloscope.trigSlope(1,"PNGReater",0.2,1.5)
    ctx.sourcemeter.rampvol(3.3,0,0.2,-0.05)#rampvol(self,start,target,de,steps):
    ...
    for i in range(0,len(porWave)):
        if float(porWave[i]) <= 0.8 :
            ctx.logger.info ('VCC %f V when POR less than 0.8' % float(vccWave[i]))
            break
        elif i == len(porWave) -1 :
            ctx.logger.info('there is no data')
            return False
